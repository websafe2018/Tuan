...
$(function () {
    ...
    $('.sort-condition').find('li').bind({
        click: function () { 
            ...
            $("#dvCurStatus").attr({ 'attrval': val, "attrname": name }).html(name+"<i class=\"icon arrow\"></i>");

            status = val;
            if (val == 1) {
                $(".sort-condition").find(".v1").attr({ 'attrval': "1", "attrname": "进行中的" }).text("进行中的");
                $(".sort-condition").find(".v2").attr({ 'attrval': "2", "attrname": "已满标的" }).text("已满标的");
            } else {
                $(".sort-condition").find(".v1").attr({ 'attrval': "2", "attrname": "已满标的" }).text("已满标的");
                $(".sort-condition").find(".v2").attr({ 'attrval': "1", "attrname": "进行中的" }).text("进行中的");
            }
            InitPageIndex();
            getInvestList(type);
            return false;
...
function setStatusVal(val) {
    if (val == 1) {
        $("#dvCurStatus").attr({ 'attrval': val, "attrname": "进行中的" });
        $("#dvCurStatus").html("进行中的<i class=\"icon arrow\"></i>"); 
    } else {
        $("#dvCurStatus").attr({ 'attrval': val, "attrname": "已满标的" });
...
//加载借款项目列表
function getInvestList(currentType) { 
    var pageindex = 1;
    if (currentType == "1") {
        $(".bdlow").show();
        $("#projectlist").html("").append(" <div attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
        pageindex = pageindex1;
    }
    else if (currentType == "2") {
        $(".bdlow").show();
        $("#weilist").html("").append(" <div attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
         pageindex = pageindex2;
    } 
    else if (currentType == "3") {//分期宝
        $(".bdlow").show();
        $("#fqbaolist").html("").append(" <div  attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
        
        pageindex = pageindex3;
    }
    else if (currentType == "4") {//供应链
        $(".bdlow").show();
        $("#gyllist").html("").append(" <div attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
        pageindex = pageindex4;
    }
    else if (currentType == "5") {//项目宝
        $(".bdlow").show();
        $("#Xmblist").html("").append(" <div attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
        pageindex = pageindex5;
    }
    else if (currentType == "6") {//资产标
        $(".bdlow").show();
        $("#jinglist").html("").append(" <div attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
        pageindex = pageindex6;
    }
    else if (currentType == "7") { //债权转让
        $(".bdlow").show();
        $("#ZqZrlist").html("").html("").append(" <div attrid='loading'><div class=\"loadingWord1\"><img alt=\"\" src=\"//js.tuandai.com/images/investlist/waiting.gif\" /></div></div>");
    ...
    $.ajax({
        ...
        success: function (json) { 
            ...
            var source;
            if (currentType == "1") { 
                $('#projectlist').html("").append(json.projectListHtml);
            }
            else if (currentType == "2") { 
                $('#weilist').html("").append(json.projectListHtml);
            } 
            else if (currentType == "3") { 
                $('#fqbaolist').html("").append(json.projectListHtml);
            }
            else if (currentType == "4") { 
                $('#gyllist').html("").append(json.projectListHtml);
            }
            else if (currentType == "5") { 
                $('#Xmblist').html("").append(json.projectListHtml);
            }
            else if (currentType == "6") {               
                $('#jinglist').html("").append(json.projectListHtml);
            }
            else if (currentType == "7") { 
                $('#ZqZrlist').html("").append(json.projectListHtml);
            }
            ...
            investInitActive();
            initSubscribesPagin(json.total, currentType);
            $("div[attrid='loading']").remove();
        },
