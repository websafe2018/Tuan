...
function loadtab(list) {
    ...
    for (var i = 1; i <= curdays; i++) {
        ...
        t = year + "-" + strmonth + (i < 10 ? "-0" + i : "-" + i);
        d = "";
        if (dlist != null && dlist.length > 0) {
            d = Query(t);
        }
        if (d.indexOf("回款") > 0 && d.indexOf("还款") > 0) {
            //格子里面数字的颜色
            html.push("<td  style='background:#C6E2FF;color:black'   val=" + t + " type='3'>" + i + d + "</td>");
        } else if (d.indexOf("回款") > 0 && d.indexOf("还款") < 0) {
            html.push("<td  style='background:#B4EEB4;color:black'   val=" + t + " type='2'>" + i + d + "</td>");
        } else if (d.indexOf("回款") < 0 && d.indexOf("还款") > 0) {
            html.push("<td  style='background:#FFAEB9;color:black'   val=" + t + " type='1'>" + i + d + "</td>");
        } else {
            html.push("<td style='background:#fff;color:black;' type='0'>" + i + d + "</td>");
        }
    ...
    if (enddays > 0) {
        ...
        for (var i = 1; i <= enddays; i++) {
            //html.push("<td style='color:#ddd;background:#fff;'>" + i + "</td>");
            t = yearnext + "-" + strmonthnext + (i < 10 ? "-0" + i : "-" + i);
            d = "";
            if (dlist != null && dlist.length > 0) {
                d = Query(t);
            }
            if (d.indexOf("回款") > 0 && d.indexOf("还款") > 0) {
                ...
                html.push("<td style='background:#fff;color:#ddd;' type='0'>" + i + d + "</td>");
            }
        }
        // for (var i = 1; i <= enddays; i++) {
        //     html.push("<td style='color:#ddd;background:#fff;'>" + i + "</td>");
    ...
    for (var i = 0; i < dlist.length; i++) {
        if (dlist[i].cycDate.toString() == date.toString()) {
            ...
            if (parseFloat(dlist[i].zxOutAmount) > 0) {
                d += "<br/><a href='" + __ZXWebsiteUrl + "/member/account_zxborrowReturnPlanList.aspx?date=" + sDate + "' target='_blank'>还款：" + fmoney(parseFloat(dlist[i].zxOutAmount).toFixed(2), 2) + "元</a>";
            }
            d += "</div>";
            break;
        }
