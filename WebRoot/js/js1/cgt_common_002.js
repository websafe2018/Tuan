	...
	$.ajax({
		...
		success: function (json) {
			...
			if (rtn == "5") {
				$(".xm-mask").show();
                if(window.isOpenDialWindows === true){
                    $("#dial-window,#diallayer").hide();
                }
				$('.xiamen-pop').show();
				$("#aBankBind").show();
				isopen = false;
			}
			else if (rtn == "4") {//设置银行卡信息
				$(".xm-mask").hide();
				$('.xiamen-pop').hide();
				isopen = false;
				completeBankBox();
			...
			else if (rtn == "222")
			{
				...
				$("#sp_newcontent").html('注意：您目前还未开通存管账户，暂时不能进行充值、提现等资金操作，请尽快联系团贷网客服<a href="' + srcc + '" target="_blank" class="XMCGQQ">2880761023</a>或<a target="_blank" class="XMCGQQ XMCGQQ_code" style="background:url(//js2.tdw.cn/images/member/my_capital/code20170426001.png) !important;cursor: pointer;">2880761023</a><div class="codexe" style="width:119px;height:124px;background:url(//js2.tdw.cn/images/member/my_capital/codexe.png);position:absolute;top:10px;left:213px;display: none;z-index: 100;"></div>开通。');
			...
			else if (rtn == "-2" || rtn == "-3" || rtn == "-4") {
				$(".xm-mask").show();
				$("#gangaoAlert").show();

				var iicardtype = rtn * -1;
				switch (iicardtype) {
					case 2:
						sp_title = "港澳台通行证";
