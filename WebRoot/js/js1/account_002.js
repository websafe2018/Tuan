...
$(function () {
    ...
    $('#check1').find('label').toggle(
        function () {
            $(this).css({ backgroundPosition: "-15px -214px" });
        },
        function () {
            $(this).css({ backgroundPosition: "-15px -14px" });
        }
    );
    ...
    $('.account-left').find('dt').toggle(//左边导航
        function () {
            ...
            $(this).parent().find('dt').css({ borderBottom: "none" });
            $(this).parent().find('dt').find('.arrow').addClass('arrow-change');
        },
        function () {
            $(this).parent().find('dd').slideDown("fast");
            $(this).parent().find('dt').css({ borderBottom: "1px solid #e5e5e5" });
...
function ranking() {
    $('.ranking-index').find('li').mouseenter(function () {//排名
        ...
        switch (_index) {
            case 0:
                {
                    $("#p1").html("<a href=\""+portalWebsiteUrl+"/pages/invest_list.aspx\" class=\"font-orange2 font-14 margin-top-15\">去投资提升排名</a>");
                    break;
                }
            case 1:
                {
