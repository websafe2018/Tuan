					...
					<div class="data-title clearfloat">
						<div class="t1 font-666 font-14 fl_r">
							<span class="fl_r" style="display: none;">平均利息成本： <span
								class="font-orange "> 10.00%</span></span> 累计利息： <span
					...
					<div class="account_tableBox clearfloat">
						<div class="nav_top">
							<ul class="nav_title">
								...
								<li class="tab_title_a"><a href="${pageContext.request.contextPath }/jsp/member/account_borrowInBid2.jsp">已完成借款</a></li>
							</ul>
						</div>
  ...
  /*  function GetData() {
       ...
       //删除记录：
         function delLoadRecord(projectId) {
		        	 ...
		        	 alert("id="+id)
		        	 if(dodel==true){
				       		 $.post("/Tuan/deleteLoadrecord.do",{"id":id}, function (data) {
				       		 alert("data:"+data)
				    	                 if (data == 0) {
				    	                   	alert("删除成功")
 ...
 //已完成的借款--------------------------------------------------------------------------------------------      
	...
	$.post("/Tuan/findjilu.do",{"uid":uid},function(data){
		...
		//alert("id"+data[i].id)
			...
			if(data[i].loadStatus==1){
			var loadStatus=$("<td class=\"txt_center \" ></td>").text("已审核");
			...
			var div="<div class='list-item-1' style='width: 82px; z-index: 0;'><dl><dt><a href='javascript:delLoadRecord(\""+data[i].id+"\");'>删除</a></dt></dl></div>";
